...
PROFILE_GUIDANCE: dict[str, list[str]] = {
    ...
    PROFILE_WINDOWS_CPU: [
        "Windows without a CUDA GPU in the tested path. The pipeline runs, and the",
        "deep learning phases run on CPU.",
        "",
        "Expect the order of magnitude, not the reference timings: the deep learning",
        "phase takes roughly 7 minutes on the reference GPU and tens of minutes to a",
        "few hours on CPU, and the cross-dataset phase scales the same way. It has",
        "not hung; it is training.",
        "",
        "The shell scripts (scripts/run_all.sh, scripts/download_datasets.sh) assume",
        "a POSIX shell and POSIX paths, so run them under WSL or Git Bash rather than",
        "PowerShell. Alternatively, skip them and call the stages directly:",
        "  python -m src.experiments.run_classical --all",
        "  python -m src.experiments.run_deep --all",
        "  python -m src.experiments.run_cross --all",
        "",
        "Numerical agreement with the published results is not expected on CPU for",
        "the neural models, and no Windows run is committed to this repository, so",
        "there is no Windows baseline to compare yours against yet.",
    ],
    PROFILE_UNSUPPORTED: [
