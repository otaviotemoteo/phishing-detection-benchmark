...
Every finding above survives on both. The transfer collapse keeps its shape (Windows cross-dataset F1 0.301–0.507 against the 0.301–0.520 reported here), the within-dataset baseline reproduces identically at 0.756–0.938, and the same CNN-LSTM direction still lands below 0.5 AUC. Windows agrees more closely than macOS wherever the linear algebra dominates — under the pinned `numpy==1.26.4` all three platforms run OpenBLAS, but Windows shares the x86-64 build with the Linux reference while Apple Silicon runs an arm64 build with different kernels — and no better on the neural models, which ran on CPU on both against the reference's GPU. Full analysis in [SETUP.md](SETUP.md#the-verification-run).
